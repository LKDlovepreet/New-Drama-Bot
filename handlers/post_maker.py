...
# 👇 SessionLocal zaroori hai database error bachane ke liye
...
async def process_target(message: types.Message, state: FSMContext):
    ...
    # 👇 FIX 1: Preview me alag-alag handle karna
    try:
...
async def run_broadcast(bot, data, target, admin_chat_id):
    ...
    # Naya Database Session (SSL Error hatane ke liye)
    ...
    try:
        ...
        if target in ["📢 Channels Only", "🚀 Both (All)"]:
            ...
            for ch in channels:
                try:
                    # 👇 FIX 2: Channel Broadcasting Split logic
                    msg = None
                    if data['media_type'] == 'photo':
        ...
        if target in ["👥 Users Only", "🚀 Both (All)"]:
            ...
            for user in users:
                try:
                    # 👇 FIX 3: User Broadcasting Split logic
                    if data['media_type'] == 'photo':
